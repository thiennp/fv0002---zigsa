	...
	.run(
		['$rootScope', '$state', '$stateParams', 'Auth', '$http', 
			function($rootScope, $state, $stateParams, Auth, $http) {
				...
				$rootScope.$on("$stateChangeSuccess", function(event, toState, toParams, fromState, fromParams) {
					...
					if (!$rootScope.isBacked) {
						...
						if (fromState.name) {
							...
							if (fromState.name == "app.tracking_update_list" && !$rootScope.fromMenu) {
								$rootScope.fromTrackingUpdateList = true;
							} else {
								$rootScope.fromMenu = false;
								$rootScope.fromTrackingUpdateList = false;
							}
						}
					} else {
				...
				$rootScope.goTrackingUpdate = function() {
					$rootScope.fromMenu = true;
					$state.go ("app.tracking_update");
				};
